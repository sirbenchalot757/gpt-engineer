...
def process_images_to_text(input_directory, output_directory, file_type, start_from_file=None, end_at_file=None):
    print(f"Processing files from {start_from_file} to {end_at_file} in {input_directory}")
    check_directories(input_directory, output_directory)
    sorted_filenames = get_sorted_filenames(input_directory, file_type, start_from_file, end_at_file)
    ...
    current_batch_text = ""
    current_batch_filenames = []  # To keep track of filenames in the current batch
    total_cost = 0  # Initialize total cost

    for filename in sorted_filenames:
        print(f"Processing file: {filename}")
        extracted_text, updated_filenames = process_file(input_directory, filename, current_batch_filenames, file_type)

        # Check if adding this file exceeds the max characters per batch
        if len(current_batch_text) + len(extracted_text) > max_chars_per_batch:
            print(f"Current batch text size exceeded. Processing batch...")
            ...
            # If the current file is too large, process it individually
...
def get_sorted_filenames(input_directory, file_type, start_from_file=None, end_at_file=None):
    file_extension = f".{file_type}"
    sorted_filenames = sorted(filter(lambda f: f.endswith(file_extension), os.listdir(input_directory)))
    if start_from_file:
        sorted_filenames = [f for f in sorted_filenames if f >= start_from_file]
